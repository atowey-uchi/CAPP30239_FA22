/* Bar chart for Killings by Race */

d3.csv("race_df.csv").then(data => {

    for (let d of data) {
        d.num = +d.num; //force num to be a number
    };

    data.sort((a, b) => d3.ascending(a.num, b.race)); // sort alphabetically

    const height = 600, 
          width = 800,
          margin = ({ top: 25, right: 30, bottom: 35, left: 50 });

    let svg = d3.select("#chart1") // select the element in the HTML
        .append("svg")
    ...
    bar.append('text')  // add title to bar chart and center it
        ...
        .attr('text-anchor', 'middle')
        .style('fill', 'black')
        .text("Police Killings by Race, 2015")
